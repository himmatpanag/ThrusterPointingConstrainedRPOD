...
methods(Static)
    ...
    function MassConsumption(solution,ax,dispName)
        if nargin < 2 || isempty(ax)
            fig = figure('Name','Fuel consumption'); 
            ax  = axes('Parent',fig);
        end
        ...
        grid(ax,'on'); 
        hold(ax,'on');
        title(ax,'Fuel consumption');
        xlabel(ax,'t'); 
        ylabel(ax,'Fuel consumption (g)');
...
%                 2,'-r','LineWidth',1,'MaxHeadSize',1,'DisplayName','Engine Plume')
    ...
    function OrientationAtTimeCRTBP(solution,t,ax,customPlot)
        axes(ax)
        for ii = 1:numel(t)
            p = interpn(solution.t,solution.x(:,8:10),t(ii));
            pos = interpn(solution.t,solution.x(:,1:3),t(ii));
   ...
   function hf=SixDOF_TrajCRTBP(solution)
    ...
    function hf=SixDOF_Traj(solution)
        ...
        % if solution.problemParameters.constraint.type == ELLIPSOIDAL
        %     s = PlotEllipsoidConstraint(gca, solution.problemParameters);
        %     set(s,'EdgeAlpha',0);
        %     title('Optimal trajectory with ellipsoidal target constraint')
        % else
    ...
    function hf2 = SixDOF_Traj_Animated(solution)
        ...
        else if solution.problemParameters.constraint.type == POINTING_CONSTRAINT_TYPE.thrust
