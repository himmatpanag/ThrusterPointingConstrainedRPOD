...
%-------------------------------------------------------------
numColors=40; %# of colors
nin = nargin;
error(nargchk(5,7,nin));
% Check numeric input arguments
if nin<6, % quiver(u,v) or quiver(u,v,s)
  [msg,x,y,u,v,vr] = xyzchk(varargin{1:5});
  threeDim = false;
else
    x=varargin{1};
    y = varargin{2};
    ...
    vr = varargin{7};
    msg = [];
  %[x,y,z,u,v,w,vr] = xyzchk(varargin{1:7});
  threeDim = true;
end
if ~isempty(msg), error(msg); end
scale=0; % %This is the default I think.
% if(nin == 3 | nin == 6)
...
if threeDim
    xyuvvrNN = [x(nonNaNind),y(nonNaNind),z(nonNaNind),u(nonNaNind),v(nonNaNind),w(nonNaNind),vr(nonNaNind)];
    numRows = 7;
else
    xyuvvrNN = [x(nonNaNind),y(nonNaNind),u(nonNaNind),v(nonNaNind),vr(nonNaNind)];
    numRows = 5;
end
[xyuvvrNNs, xyuvvrNNsi] = sortrows(xyuvvrNN,numRows);
iCCs=round(linspace(1,size(CC,1),numColors));
iData = round(linspace(0,size(xyuvvrNNs,1),numColors+1));
% figure;
% -------------------
...
ya = (3 * ys + yf)/4;
yb = (3 * yf + ys)/4;
% h=imagesc([xa,xb],[ya,yb],reshape(xyuvvrNNs([1,end;1,end],numRows),2,2));
% set(h,'visible','off');
caxis(xyuvvrNNs([1,end],numRows));
hold on;
for it=1:numColors
    c = CC(iCCs(it),:);
    si = iData(it)+1;
    ei = iData(it+1);
    if threeDim
        hh=quiver3(xyuvvrNNs(si:ei,1),xyuvvrNNs(si:ei,2),xyuvvrNNs(si:ei,3),xyuvvrNNs(si:ei,4),xyuvvrNNs(si:ei,5),xyuvvrNNs(si:ei,6),scale*it/numColors,'Color',c,'HandleVisibility','off');
    else
        hh=quiver(xyuvvrNNs(si:ei,1),xyuvvrNNs(si:ei,2),xyuvvrNNs(si:ei,3),xyuvvrNNs(si:ei,4),scale*it/numColors,'Color',c);
    end
end    
colorbar;
%--- commented out from quiverc2 -------------------------------------------
